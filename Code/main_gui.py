import filters as filters1
import pencil_sketch_filter as filters2
import detector_function as detect
...
# create objects
[ok, frame] = cap.read()
shadow = filters1.videoShadow(buffer_size=16, delay_frames=3, alpha=0.5, frame_size=frame.shape)
faceSticker = filters1.faceSticker_landmarks("../Image/sticker_file_0.txt")
...
while cap.isOpened():
    ...
    i = effect_var.get()
    if i == 1:
        frame_out = filters2.Sketch(frame_out)
    elif i == 2:
        frame_out = filters2.BlackWhite(frame_out)
    elif i == 3:
        frame_out = filters2.Cartoon(frame_out)
    elif i == 4:
        frame_out = filters1.filterRGBGlitch_Random(frame_out)
    elif i == 5:
    ...
    for rect in rects:
        ...
        blinked_rect, COUNTER = detect.detect_eye_blink(landmarks, COUNTER)
        # if blinked_rect:
        #     print("blinked")
